...
public class BalisticCalculator : MonoBehaviour {
    ...
    [SerializeField] bool isHighArc;
    [SerializeField] float maxVelocityImpulse = 0;
    [SerializeField] BalisticCalculator otherLeg;
    int goalIndex;
	// Use this for initialization
    float t = 1;
	void Start () {
		//transform.GetComponent<Rigidbody>().velocity=calcBallisticVelocityVector(transform.position, new Vector3(0,0,0), 45);
	...
	void Update () {
        if (Vector3.Distance(goals[goalIndex].position, transform.position) < 0.5)
        {
            GetComponent<Rigidbody>().drag = float.MaxValue;
            hasLaunched = false;
            goalIndex ++;
            goalIndex=Mathf.Min(goalIndex, goals.Length-1);
            otherLeg.enabled = true;
            this.enabled = false;
        } else
        {
            if(hasLaunched == false) { 
                ...
                Vector3 high, low;
                fts.solve_ballistic_arc(transform.position, maxVelocityImpulse, goals[goalIndex].position, Physics.gravity.magnitude, out high, out low);
                if (isHighArc) {
                    transform.GetComponent<Rigidbody>().velocity = high;
                } else
                {
                    transform.GetComponent<Rigidbody>().velocity = low;
                }
                hasLaunched = true;
            }
        }
...
public class fts
{
    ...
    public static float ballistic_range(float speed, float gravity, float initial_height)
    {
        ...
        // Footling these cases is up to your project's coding standards
    ...
    public static int solve_ballistic_arc(Vector3 proj_pos, float proj_speed, Vector3 target, float gravity, out Vector3 high, out Vector3 low)
    {
        ...
        high = groundDir * Mathf.Cos(lowAng) * proj_speed + Vector3.up * Mathf.Sin(lowAng) * proj_speed;
        if (numSolutions > 1)
            low = groundDir * Mathf.Cos(highAng) * proj_speed + Vector3.up * Mathf.Sin(highAng) * proj_speed;
